...
var EtcManager = function(etcConfig, actor) {
	...
	if (etcdNode.dir == true) {
		...
		for (var index in etcdNode.nodes) {
			...
			var key = childNode.key
				.replace(dirKey, "") // remove dir prefix key
			...
			$this.getKey($this.getDirName())
				.then(function(value) {
					if (value == null) {
						...
						logger.info(util.inspect($this.listeners('onActorsChanged')))
...
EtcManager.prototype.registerActor = function(actorMgr) {
	...
	return new Promise((resolve, reject) => {
		...
		return $this.setKey($this.getKeyName(), jsonValue).then(function(setRet) {
				...
				return $this.setKey($this.getLastChangedKey(), _.now());
			});
	});
}
